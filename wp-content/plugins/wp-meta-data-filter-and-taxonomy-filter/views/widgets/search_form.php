...
<p>
    <label for="<?php echo $widget->get_field_id(MetaDataFilterCore::$slug_cat); ?>"><?php _e('Meta Data Filter Category', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
    <?php
    wp_dropdown_categories(
            array(
                ...
                'show_option_none' => __('select category', 'wp-meta-data-filter-and-taxonomy-filter'),
    ...
    <i><?php _e('You can check more filter categories to enable changing of it on front!', 'wp-meta-data-filter-and-taxonomy-filter') ?></i>
<?php endif; ?>
</p>
...
<p>
    ...
    <label for="<?php echo $widget->get_field_id('hide_meta_filter_values'); ?>"><?php _e('Hide meta values filter options', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
<div style="background: yellow; padding: 3px;"><?php _e('Hide meta values filter options necessarily if you are going to filter by taxonomies <b>only</b>!', 'wp-meta-data-filter-and-taxonomy-filter') ?></div>
</p>
...
<p>
    ...
    <label for="<?php echo $widget->get_field_id('hide_tax_filter_values'); ?>"><?php _e('Hide taxonomies values filter options', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
</p>
...
<h3><?php _e('Meta options', 'wp-meta-data-filter-and-taxonomy-filter') ?></h3>


<p style="display:none;">
    <label for="<?php echo $widget->get_field_id('and_or'); ?>"><?php _e('AND/OR behaviour for meta filelds', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
    <?php
    $sett = array(
        'AND' => __('AND', 'wp-meta-data-filter-and-taxonomy-filter'),
        'OR' => __('OR', 'wp-meta-data-filter-and-taxonomy-filter'),
            //'BOTH'=>__('show AND/OR filter', 'wp-meta-data-filter-and-taxonomy-filter'),
    );
    ?>
...
<p>
    ...
    <label for="<?php echo $widget->get_field_id('show_checkbox_items_count'); ?>"><?php _e('Show near checkbox count of posts', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
</p>
...
?>
    ...
    <label for="<?php echo $widget->get_field_id('show_select_items_count'); ?>"><?php _e('Show count of posts in select option', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
</p>
...
?>
    ...
    <label for="<?php echo $widget->get_field_id('show_slider_items_count'); ?>"><?php _e('Show count of posts in range slider', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
</p>
...
?>
    ...
    <label for="<?php echo $widget->get_field_id('show_items_count_dynam'); ?>"><?php _e('Dynamic post items recount', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
</p>
...
?>
    ...
    <label class="mdf_for_premium_label" for="<?php echo $widget->get_field_id('hide_items_where_count_0'); ?>"><?php _e('Hide items where count of items is 0', 'wp-meta-data-filter-and-taxonomy-filter') ?>: (<a href="https://pluginus.net/affiliate/meta-data-taxonomies-filter" target="_blank">premium version</a>)</label>
</p>
...
?>
    ...
    <label for="<?php echo $widget->get_field_id('show_filter_button_after_each_block'); ?>"><?php _e('Show filter button after each block', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
</p>
...
if (!empty($taxonomies))
{
    ...
    <div style="background: yellow; padding: 3px;"><?php _e('To show slugs taxonomies you have to add in selected Meta Data Filter Category a data constructor block of type <b>"taxonomy"</b>. It must appear one time only in search form of one widget! Otherwise maybe problems.', 'wp-meta-data-filter-and-taxonomy-filter'); ?></div>
        <?php
    } else
    {
        echo '<br />';
...
<p>
    <label for="<?php echo $widget->get_field_id('title_for_any'); ?>"><?php _e('Title for Any', 'wp-meta-data-filter-and-taxonomy-filter') ?>:</label>
